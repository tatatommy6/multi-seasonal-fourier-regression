...
def load_datasets(csv_path: str):
    ...
    # 컬럼 체크는 이제 df.columns 기준 (Datetime은 index로 갔으니 체크 대상 아님)
    ...
    # 시간축 (1시간 단위 샘플 인덱스)
    t = torch.arange(y.shape[0], dtype=torch.float32).unsqueeze(1)
    X = t.repeat(1, 3)
...
def main():
    ...
    X, y, _, _ = load_datasets(csv_path) # https://github.com/tatatommy6/multi-seasonal-fourier-regression/issues/7
